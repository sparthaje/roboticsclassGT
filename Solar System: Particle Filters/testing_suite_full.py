...
# Required to pass at least 7 test cases to earn full credit.
# Part A is worth 70% and Part B is worth 30%.
...
class CaseRunner(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        try:
            ...
            raise Exception(f"Error calling who_am_i(): {e}")
        ...
        if student_id:
            ...
            try:
                for part_name, suite in zip(suite_names, suites):
                    ...
                    if part_name == "A":
                        total_passes_A += num_passes
                    elif part_name == "B":
                        total_passes_B += num_passes

                    print("Successes: {}\nFailures: {}\n".format(num_passes, num_errors + num_fails))

                # Part A - 70%, Part B - 30%
                score_A = round(((float(min(TESTS_REQUIRED, total_passes_A)) / TESTS_REQUIRED) * WEIGHT_A), 2)
                score_B = round(((float(min(TESTS_REQUIRED, total_passes_B)) / TESTS_REQUIRED) * WEIGHT_B), 2)
                print("====================\nScore Part A: {}/70.0\nScore Part B: {}/30.0\n".format(score_A, score_B))
                overall_score = round(score_A + score_B, 2)
